...
class G_NET(nn.Module):
	...
	def forward(self, z_code, c_code, p_code=None, bg_code=None):
		...
		if cfg.TIED_CODES:
			p_code = child_to_parent(c_code, cfg.FINE_GRAINED_CATEGORIES,
			                         cfg.SUPER_CATEGORIES)  # Obtaining the parent code from child code
			bg_code = c_code

		# Background stage
		h_code1_bg = self.h_net1_bg(z_code, bg_code)
		fake_img1 = self.img_net1_bg(h_code1_bg)  # Background image
		fake_img1_126 = self.scale_fimg(
			fake_img1)  # Resizing fake background image from 128x128 to the resolution which background discriminator expects: 126 x 126.
...
class D_NET(nn.Module):
	...
	def forward(self, x_var):
		...
		if self.stg_no == 0:
			...
			classi_score = self.uncond_logits1(
				x_code)  # Background vs Foreground classification score (0 - background and 1 - foreground)
			...
			code_pred = self.logits(
				h_c_code)  # Predicts the parent code and child code in parent and child stage respectively
