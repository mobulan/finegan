...
def define_optimizers(netG, netsD):
	...
	for i in range(num_Ds):
		if i == 1:
			...
			opt = optim.Adam(
				[{'params': netsD[i].module.jointConv.parameters()}, {'params': netsD[i].module.logits.parameters()}],
				lr=cfg.TRAIN.GENERATOR_LR,
				betas=(0.5, 0.999))
			optimizerG.append(opt)
...
class FineGAN_trainer(object):
		...
		if idx == 0 or idx == 2:  # Discriminator is only trained in background and child stage. (NOT in parent stage)
			...
			if idx == 2:
				...
				for i in range(batch_size):
					...
					b2 = min(torch.tensor(self.n_out - 1).float().cuda(),
				...
				norm_fact_fake = weights_real.shape[0] * weights_real.shape[1] * weights_real.shape[2] * \
			...
			if idx == 0:  # Background stage
				...
				errD_real_uncond = criterion(real_logits[1],
				                             real_labels)  # Real/Fake loss for 'real background' (on patch level)
				errD_real_uncond = torch.mul(errD_real_uncond,
				                             weights_real)  # Masking output units which correspond to receptive fields which lie within the boundin box
				errD_real_uncond = errD_real_uncond.mean()

				errD_real_uncond_classi = criterion(real_logits[0],
				                                    weights_real)  # Background/foreground classification loss
				errD_real_uncond_classi = errD_real_uncond_classi.mean()

				errD_fake_uncond = criterion(fake_logits[1],
				                             fake_labels)  # Real/Fake loss for 'fake background' (on patch level)
				errD_fake_uncond = errD_fake_uncond.mean()

				if (
						norm_fact_real > 0):  # Normalizing the real/fake loss for background after accounting the number of masked members in the output.
		...
		for i in range(self.num_Ds):
			...
			if i == 0 or i == 2:  # real/fake loss for background (0) and child (2) stage
				...
				if i == 0:
					...
					errG_classi = criterion_one(outputs[0],
					                            real_labels)  # Background/Foreground classification loss for the fake background image (on patch level)
	...
	def train(self):
		...
		self.patch_stride = float(
			4)  # Receptive field stride given the current discriminator architecture for background stage
		...
		for epoch in range(start_epoch, self.max_epoch):
			...
			for step, data in enumerate(self.data_loader, 0):
				...
				if count % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:
					...
					save_img_results(self.imgs_tcpu, (self.fake_imgs + self.fg_imgs + self.mk_imgs + self.fg_mk),
					                 self.num_Ds,
		...
		for step, data in enumerate(self.data_loader, 0):
			...
			if (count % 2) == 0:  # Train on normal batch of images
				...
				all_hard_class = Variable(
					torch.zeros(self.batch_size * repeat_times, cfg.FINE_GRAINED_CATEGORIES)).cuda()
				...
				for hard_it in range(repeat_times):
					...
					self.fake_imgs, self.fg_imgs, self.mk_imgs, self.fg_mk = self.netG(hard_noise.detach(),
					                                                                   hard_class.detach())
...
class FineGAN_evaluator(object):
	...
	def evaluate_finegan(self):
		if cfg.TRAIN.NET_G == '':
			...
			fake_imgs, fg_imgs, mk_imgs, fgmk_imgs = netG(noise, c_code, p_code,
